use test;
#---------------------------------------
create procedure deptempc(IN param1 INT, OUT param2 INT) SELECT COUNT(*) INTO param2 FROM emp where emp.deptno=param1;

create procedure empcomm(IN param1 INT, OUT param2 INT) SELECT COUNT(*) INTO param2 FROM emp where emp.deptno=param1 and emp.comm is not NULL;
...
write sql procedure to retireve all emp with theoir deppt names
*/
DELIMITER //
...
/*
write a sql procedure to find employees earning above 2500 abd return their count
(in min salary)
ename sal deptno
*/
DELIMITER //
CREATE PROCEDURE minsal(IN min_sal double, )
BEGIN 
select ename, count(empno) as name_count from emp e group by ename having count(ename)>1;
